...
int main(int argc, char *argv[]) {
    ...
    // Allocate memory - use a large portion of available memory (80%)
    size_t availableMem = prop.totalGlobalMem * 0.8;
    ...
    CHECK_CUDA_ERROR(cudaMalloc((void**)&d_data1, memSize));
    CHECK_CUDA_ERROR(cudaMalloc((void**)&d_data2, memSize));
    CHECK_CUDA_ERROR(cudaMalloc((void**)&d_data3, memSize));
    
    // Copy data from host to device
    CHECK_CUDA_ERROR(cudaMemcpy(d_data1, h_data, memSize, cudaMemcpyHostToDevice));
    CHECK_CUDA_ERROR(cudaMemcpy(d_data2, h_data, memSize, cudaMemcpyHostToDevice));
    ...
    // Optional: Set device to maximum clock speeds if supported
    // Note: This requires admin privileges and might not work on all systems or GPUs
    system("nvidia-smi -ac 1593,1980"); // H100 clocks for memory,graphics
    ...
    CHECK_CUDA_ERROR(cudaMemcpy(h_data, d_data1, memSize, cudaMemcpyDeviceToHost));
    ...
    CHECK_CUDA_ERROR(cudaFree(d_data3));
